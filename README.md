...
The Commandline Interpreter can be started by executing the command `./console.py`. The `console` can `create`, `destroy`, and `update` objects. Type `help` within the console to get a list of command options and their functions.
...
| models/engine/ | file_storage.py | Serializes instances to a JSON file and deserializes a JSON file to instances |
| updated | Updates an instance by its class name and ID, adding or modifying an attribute, and saves the changes in a JSON file.
